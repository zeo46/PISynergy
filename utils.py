...
from dataset.synergyZ_dataset import MyInMemoryDataset
...
def load_dataloader(n_fold,args):
    ...
    if args.celldataset == 1:
        ...
        celllines_data = osp.join(data_root,'cell_data/4079g/985_cellGraphs_exp_mut_cn_eff_dep_met_4079_genes_norm.npy')
    elif args.celldataset == 3:
        celllines_data = osp.join(data_root,'cell_data/963g/985_cellGraphs_exp_mut_cn_963_genes.npy')

    # osp 函数用于路径拼接文件路径
    drugs_data = osp.join(data_root,'drug_data/drugSmile_drugSubEmbed_2644.npy')
    ...
    tr_dataloader = DataLoader(tr_dataset, batch_size=args.batch_size, shuffle=True, num_workers=1, drop_last=True)
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=1, drop_last=True)
    test_dataloader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=1, drop_last=True)
